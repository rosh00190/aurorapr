...
:}`;
            ...
            const settingsMenuLabels = ['"🌱\\{\\{char\\}\\}(기본값)"', ...bookmarkLabels, JSON.stringify("➕ 북마크 추가"), JSON.stringify("🗑️ 북마크 삭제"), JSON.stringify("수동설정"), JSON.stringify("↩️ 목록으로 돌아가기")].join(', ');
    ...
    `, `/if left={{var::choice}} right="➕ 북마크 추가" rule=eq {: /:addBookmark :} | 
                ...
                /if left={{var::choice}} right="↩️ 목록으로 돌아가기" rule=eq {: /:mainMenu :}`].join(' | \n');
...
:}`;
            ...
            for (const category of menuConfig.categories) {
                const subMenuLabels = [...category.items.map(item => JSON.stringify(item.name)), JSON.stringify("↩️ 목록으로 돌아가기")].join(', ');
...
:}`;
            ...
            const mainIfClauses = [
                ...
                `/if left={{var::choice}} right="⚙️ 언어 설정" rule=eq /:auroralangSettingsMenu`,
