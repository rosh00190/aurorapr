...
:}`;
    ...
    /if left={{getglobalvar::orora_char_bookmarks}} right=[] rule=neq else={: /echo "ℹ️ 삭제할 북마크가 없습니다." severity=info || /:auroraSettingsMenu :} {:
...
:}`;
    ...
    `,
                ...
                /if left={{var::choice}} right="↩️ 메인메뉴로 돌아가기" rule=eq {: /echo 메인메뉴로 || /:mainMenu :}`
            ].join(' | \n');
...
`;
            ...
            for (const category of menuConfig.categories) {
                ...
                subMenuIfs.push(`/if left={{var::choice}} right="↩️ 메인 메뉴로 돌아가기" rule=eq {: /:mainMenu :}`);
...
:}`;
    ...
    /if left={{pipe}} right="" rule=eq {: /setglobalvar key=orora_char_bookmarks [] | /echo "북마크 변수 초기화 완료." :} |
    ...
    async function handleWorldInfoUpdate() {
        ...
        debounceTimer = setTimeout(async () => {
            try {
                ...
                logger.info(`오로라 트리거 감지됨: [${selectedFile}]`);
...
*/
                ...
                if (middlePrompt.includes('//# INTERACTIVE_MODULE')) {
                    //const interactiveUrl = getProxiedUrl(`orora/${command}.txt`);
                    const interactiveUrl = getProxiedUrl('interactive_module_prompt.txt');
                    interactivePrompt = await getPromptText(interactiveUrl) || '';
                }
    ...
    // 2. Windows 줄바꿈(\r\n)을 표준 줄바꿈(\n)으로 통일합니다.
                ...
                if (fixedChar && fixedChar !== '{{char}}') {
                    logger.info(`고정 캐릭터 [${fixedChar}](으)로 {{char}}를 치환합니다.`);
                    finalCleanPrompt = finalCleanPrompt.replaceAll('{{char}}', fixedChar);
                }

                logger.info("✅ 최종 프롬프트 정규화 완료. AI 생성 요청...");
    ...
    eventOnButton(BUTTON_NAME, async () => {
        ...
        if (!isInitialized) {
            logger.debug(`캐릭터 초기화 실패로 메뉴 생성을 중단합니다.`);
            return;
        }
        createDynamicMenu();
    });
