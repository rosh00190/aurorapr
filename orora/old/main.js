...
(function() {
    ...
    const IS_TESTER_MODE = true; // ▼▼▼▼▼ [수정됨] 테스터 모드 플래그 신설 ▼▼▼▼▼
...
if (DEBUG_MODE && IS_TESTER_MODE) { // 테스터 모드일 때 알림 추가
    ...
    // 2. 함수 다운로드 및 생성 함수
    async function loadExternalFunction(functionName) {
        // 이미 캐시에 있으면 바로 리턴
        if (functionCache[functionName]) {
            logger.debug(`캐시된 함수 사용: ${functionName}`);
            return functionCache[functionName];
        }

        try {
            // 함수 파일 다운로드 (기존 getPromptText 사용!)
            const functionUrl = getProxiedUrl(`functions/${functionName}.js`);
            logger.debug(`함수 다운로드 시작: ${functionUrl}`);
            
            const functionCode = await getPromptText(functionUrl);
            if (!functionCode) {
                throw new Error(`함수 코드가 비어있음: ${functionName}`);
            }

            // 다운로드한 텍스트를 실제 함수로 변환
            const loadedFunction = new Function('return ' + functionCode)();
            
            // 캐시에 저장
            functionCache[functionName] = loadedFunction;
            logger.debug(`함수 로딩 완료: ${functionName}`);
            
            return loadedFunction;
            
        } catch (error) {
            logger.error(`함수 로딩 실패: ${functionName}`, error);
            toastr.error(`${functionName} 함수를 불러올 수 없습니다.`);
            return null;
        }
    }
...
/*
    ...
    */
    ...
    // 3. 기존 processCustomRandom 함수를 교체할 새로운 함수
    async function processTextWithExternalFunction(text) {
        // 외부 함수 다운로드
        const randomProcessor = await loadExternalFunction('random_processor');
        
        if (!randomProcessor) {
            // 함수 로딩 실패시 원본 텍스트 그대로 리턴
            logger.warn('랜덤 처리 함수 로딩 실패, 원본 텍스트 사용');
            return text;
        }

        // 외부 함수 실행
        return randomProcessor(text);
    }
...
*/
               ...
               // --- 이 아래 코드로 기존 로직을 교체하세요 ---
                ...
                // [신규] 스크립트 자체 랜덤 처리.
                processedMiddlePrompt = await processTextWithExternalFunction(processedMiddlePrompt);
